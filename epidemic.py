...
class SI(Model):
    ...
    def epidemic(self, infected_M, infected_N, infected_P, S_List, all_nodes):
        ...
        next_S_set_1 = self.change_state_1(
            self.search_nearest_neighbor_1(set(S_List[1]), 1), self.infected_rate, 1
        )
        print("epidemic_next_S_set_1 ", next_S_set_1)
        # 这个函数是 S_List[2]代表已知信息片段2的列表，寻找未知信息片段2的节点，按照一定的概率将其感染为已知信息片段2
        next_S_set_2 = self.change_state_1(
            self.search_nearest_neighbor_1(set(S_List[2]), 2), self.infected_rate, 2
        )
        next_S_set_3 = self.change_state_1(
            self.search_nearest_neighbor_1(set(S_List[3]), 3), self.infected_rate, 3
        )
        next_S_set_4 = self.change_state_1(
            self.search_nearest_neighbor_1(set(S_List[4]), 4), self.infected_rate, 4
        )
        next_S_set_5 = self.change_state_1(
            self.search_nearest_neighbor_1(set(S_List[5]), 5), self.infected_rate, 5
        )
        # 最新一轮已知信息片段1的节点+最新一轮已知信息片段2的节点+最新一轮已知信息片段3的节点+最新一轮已知信息片段4的节点+最新一轮已知信息片段5的节点
        new_change_set = (
            next_S_set_1 | next_S_set_2 | next_S_set_3 | next_S_set_4 | next_S_set_5
        )
        ...
        # new_infected_1 = self.change_state(
        #     self.search_nearest_neighbor(infected_M, "S"),
        #     scale=self.infected_rate,
        #     to_state="Im",
        # )

        new_infected_M = self.change_state(
            self.search_nearest_neighbor(infected_M, "S"),
            scale=self.infected_rate,
            to_state="Im",
        )
