...
class Model(object):
    ...
    def infomation_acount_math(self,graph,i):
        ...
        return q
    # 统计节点i的稀有性
    def infomation_rarity_math(self, graph, i):
        r = 0
        if graph.nodes[i]['mass_frag'][0][0].count(1) != 0:
        ...
        if graph.nodes[i]['mass_frag'][0][0].count(5) != 0:
            ...
            graph.nodes[i]['mass_rarity'] = 0
        graph.nodes[i]['mass_rarity'] = r
        # 保留四位小数
        r = round(r, 4)
        return r
    def set_initial_condition(self):
        ...
        # 初始化节点的各个参数
        for i in self.nodes:
            self.graph.nodes[i]['node state'] = 'S'
            # 将符合幂律分布(帕累托分布)的数据赋给5000个节点'
            self.graph.nodes[i]['active'] = A[i]
            # 节点i已知的信息片段列表，初始为空
            self.graph.nodes[i]['mass_frag'] = []
           # 节点i的信息耦合值
            self.graph.nodes[i]['mass_count'] = 0
            #  节点i的信息稀有性
            self.graph.nodes[i]['mass_rarity'] = 0
            #  节点i的自我认知产生的情感值
            self.graph.nodes[i]['node_Es'] = 0
            #  周围邻居节点对节点i的情绪值的影响
            self.graph.nodes[i]['node_En'] = 0
            # 节点i的情感值
            self.graph.nodes[i]['emotion_value'] = 0
        ...
        for i in infected_M :
            ...
            #计算节点i的信息量
            self.graph.nodes[i]['mass_count']= self.infomation_acount_math(self.graph, i)
            # print('count',self.graph.nodes[i]['mass_count'])
            # 计算节点i的稀有性
            self.graph.nodes[i]['mass_rarity'] = self.infomation_rarity_math(self.graph, i)
        ...
        # S_List[0]
        #S_List=[[[],[],,,,,[]],   [], [],[],[]]
        # 假设 s_list[1][0] ，s_list[2][0]
        #   s_list[0][0]   [1]
        print('S_List',self.S_List)
        print('S_List[0]', self.S_List[0])
        print('S_List【】[1]', self.S_List[1])
        #     print('S_List[1]', self.S_List[1])
        # IndexError: list index out of range
        print('S_List[3]', self.S_List[3])
        print('S_List[0][0]', self.S_List[0][0])
    ...
    def search_nearest_neighbor(self, source_set, state):
        ...
        for node in source_set:
            target.update([v for v in self.graph.neighbors(node) if  self.graph.nodes[v]["node state"]  == state])
        else:
            if len(target) == 0:
    ...
    def change_state(self, nodes, scale, to_state):
       ...
       """
        to_nodes = random.sample(nodes, int(len(nodes) * scale)) # 从nodes中取int(len(nodes) * scale)个元素
    ...
    def spread_one_time(self,infected_M,infected_N,infected_P,S_List,seed=None):
        ...
        while(t):
            ...
            #     frag_num += 1
            # zx
            infected_M, infected_N, infected_P,one_S_List,= self.epidemic(infected_M,infected_N,infected_P,S_List[40-t],self.nodes)
            ...
            t -= 1
            self.__t += 1
        return np.asarray(infected_M_distance), np.asarray(infected_P_distance), np.asarray(infected_N_distance)

    def spread_n_time(self, n, seed):

        # """重复n次实验，每一次实验中的迭代次数为t

        n_infected_M_distance, n_infected_N_distance,n_infected_P_distance= np.zeros(self.t + 1), np.zeros(self.t + 1) ,np.zeros(self.t + 1)
        print('n',n)
        while(n):

            infected_M, infected_N,infected_P,S_List = self.set_initial_condition()
            n_infected__M_distance_temp, n_infected__N_distance_temp, n_infected__P_distance_temp, = self.spread_one_time(infected_M, infected_N,infected_P, S_List,seed=seed)
    ...
    def spread(self, seed=None, flags=False):
        ...
        self.flags = flags
        # 初始化数据
        infected_M_distance, infected_N_distance,infected_P_distance, = np.zeros(self.t + 1), np.zeros(self.t + 1), np.zeros(self.t + 1)
        ...
        number = self.n
        infected_M_distance_temp, infected_N_distance_temp,infected_P_distance_temp= self.spread_n_time(number, seed)
